...
basedir="/opt/sourcetracker-1.0.1"

metadata <- read.table(file.path(basedir, 'data/metadata.txt'),
...
otus <- read.table(file.path(basedir, 'data/otus.txt'),
                   sep='\t', header=T,row.names=1,check=F,skip=1,comment='')
...
source(file.path(basedir,'src/SourceTracker.r'))
```
